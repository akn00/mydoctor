...
const Index = () => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < qualification.length; i++) {
      if (
        ...
        String(qualification[i].name).length < 3 ||
        String(qualification[i].institute).length < 3 ||
        String(qualification[i].year).length < 4
      ) {
        setDisableSave(true);
        break;
      } else {
  ...
  useEffect(() => {
    if(editable &&qualification.length===0){
  ...
  return (
    <div style={{ display: "flex" }}>
      ...
      <div style={{ padding: "32px", width: "100%", minHeight: "73vh" }}>
        ...
        {!editable && qualification.length===0 && (
          <div
            style={{
              color: "#696969",
              display: "flex",
